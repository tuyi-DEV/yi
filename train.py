...
def parse_args():
    ...
    parser.add_argument('--val-batch-size', type=int, default=32, help='Batch size for validation')
    parser.add_argument('--epochs', type=int, default=80, help='Total number of epochs')
    parser.add_argument('--freeze-epochs', type=int, default=20, help='Number of epochs for freezing backbone')
    ...
    parser.add_argument('--train-size-limit', type=int, default=1500, help='Limit training set size')
    # 其他参数
    parser.add_argument('--save-dir', type=str, default='logs', help='Directory to save logs and weights')
    parser.add_argument('--save-period', type=int, default=10, help='Save weights every N epochs')
    ...
    parser.add_argument('--freeze-train', action='store_true', default=True, help='Whether to use freeze training')
    parser.add_argument('--lr-decay-type', type=str, default='cos', choices=['step', 'cos'], help='Learning rate decay type')
    parser.add_argument('--mosaic', action='store_true', default=False, help='Use mosaic data augmentation')
    parser.add_argument('--autoaugment', action='store_true', default=True, help='Use auto data augmentation')
...
if __name__ == "__main__":
    ...
    #   设置为None或0表示使用全部训练集
    #   否则使用指定数量的训练样本
    train_size_limit = 1500  # 适当增加训练集大小，平衡性能和时间
    #------------------------------------------------------------------#
    #   冻结阶段训练参数
    ...
    #------------------------------------------------------------------#
    Init_Epoch          = 0
    Freeze_Epoch        = 20  # 减少冻结训练轮数
    Freeze_batch_size   = 64  # 增加冻结阶段batch_size
    #------------------------------------------------------------------#
    #   解冻阶段训练参数
    ...
    #------------------------------------------------------------------#
    UnFreeze_Epoch      = 100  # 减少总训练轮数
    Unfreeze_batch_size = 32  # 适当的解冻阶段batch_size
    #------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    optimizer_type      = "adam"  # 使用Adam优化器，收敛更快
    momentum            = 0.9
    weight_decay        = 0 if optimizer_type == 'adam' else 5e-4
    #------------------------------------------------------------------#
    #   lr_decay_type   使用到的学习率下降方式，可选的有step、cos
    #------------------------------------------------------------------#
    lr_decay_type       = args.lr_decay_type  # 使用命令行参数中的学习率下降方式
    #------------------------------------------------------------------#
    ...
    #                   内存较小的电脑可以设置为2或者0  
    #------------------------------------------------------------------#
    num_workers         = 4  # CPU训练时的最佳线程数，避免线程过多导致性能下降
    ...
    #------------------------------------------------------------------#
    if fp16 and Cuda and torch.cuda.is_available():
    ...
    # 如果验证集为空，从训练集中随机分割10%作为验证集
    if num_val == 0:
        split_ratio = 0.1
        split_index = int(num_train * (1 - split_ratio))
        np.random.seed(10101)
        np.random.shuffle(train_lines)
        np.random.seed(None)
        val_lines = train_lines[split_index:]
        train_lines = train_lines[:split_index]
        num_train = len(train_lines)
        num_val = len(val_lines)
        print(f"验证集为空，从训练集中分割{split_ratio*100}%作为验证集")
        print(f"分割后：训练集{num_train}张，验证集{num_val}张")
    else:
        np.random.seed(10101)
        np.random.shuffle(train_lines)
        np.random.seed(None)
    ...
    # 限制训练集大小
    if train_size_limit and train_size_limit > 0 and len(train_lines) > train_size_limit:
        np.random.seed(10101)
        train_lines = np.random.choice(train_lines, size=train_size_limit, replace=False).tolist()
        np.random.seed(None)
        num_train = len(train_lines)
        print(f"训练集大小已限制为{train_size_limit}张")
    ...
    if True:
        ...
        #---------------------------------------#
        #   获得学习率下降的公式
        #   total_iters应该是总的迭代次数（steps），而不是总的epoch数
        #---------------------------------------#
        epoch_step = num_train // batch_size
        total_iters = UnFreeze_Epoch * epoch_step
        lr_scheduler_func = get_lr_scheduler(lr_decay_type, Init_lr_fit, Min_lr_fit, total_iters)
        ...
        train_dataset   = DataGenerator(train_lines, input_shape, True, autoaugment_flag=args.autoaugment, mosaic_flag=args.mosaic)
        val_dataset     = DataGenerator(val_lines, input_shape, False, autoaugment_flag=False, mosaic_flag=False)
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            if epoch >= Freeze_Epoch and not UnFreeze_flag and Freeze_Train:
                ...
                #   total_iters应该是总的迭代次数（steps），而不是总的epoch数
                #---------------------------------------#
                total_iters = (UnFreeze_Epoch - Freeze_Epoch) * epoch_step
                lr_scheduler_func = get_lr_scheduler(lr_decay_type, Init_lr_fit, Min_lr_fit, total_iters)
            ...
            # 开始训练并获取验证准确率
            val_accuracy = fit_one_epoch(model_train, model, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir, local_rank)
